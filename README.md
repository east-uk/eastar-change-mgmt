...
2. **첫 접속 시 자동으로 비밀번호 `eastar2026` 으로 시드**됨
3. 로그인 후 → ★ **즉시 비밀번호 변경 권장** (현재 UI 미구현: Firestore 콘솔에서 `config/admin.passwordHash` 를 새 SHA-256 해시로 교체. v2에서 비밀번호 변경 UI 추가 예정.)
...
현재 Firestore Rules는 익명 사용을 우선해 관리자 write 경로를 열어둠. 다음 보강 권장:

- **단기**: Firestore Rules의 `topics/* write`, `config/* write`를 `if false` 로 닫고 → 주제 등록·삭제는 Apps Script 또는 Cloud Function 경유
- **중기**: Firebase Anonymous Auth + custom claim `admin: true` 로 관리자 식별 → Rules에서 `request.auth.token.admin == true` 검증
